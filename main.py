...
def add_user() -> None:
    name = entry.imie.get()
    surname = entry.nazwisko.get()
    location = entry.miejscowosc.get()
    posts = entry.posty.get()
...
def edit_user() -> None:
    ...
    button_dodaj_obiekt.config(tekst='zapisz', command= lambda:update_users(i))
...
def update_users(i) -> None:
    ...
    button_dodaj_obiekt.config(tekst='dodaj', command= add_user)
...
label_pokaz_szczegoly = Label(ramka_szczegoly_obiektow, text="Szczegoly użytkownika:")
label_pokaz_szczegoly.grid(row=0, column=0)

label_szczegoly_obiektu_name = Label(ramka_szczegoly_obiektow, text='imie ')
...
label_szczegoly_obiektu_posts = Label(ramka_szczegoly_obiektow, text='posty ')
